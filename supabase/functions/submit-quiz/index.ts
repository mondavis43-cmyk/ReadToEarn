...
serve(async (req) => {
  ...
  // SECURITY FIX #16: Enhanced disposable email check (API + local fallback)
  const isDisposable = await isDisposableEmail(user.email ?? "");
  if (isDisposable) {
    return new Response(
      JSON.stringify({ error: "Disposable email addresses are not allowed. Please use a personal or business email." }),
  ...
  if (sprint_id) {
    ...
    if (isSuspicious) {
      ...
      await adminClient.from("payout_logs").insert({ user_id: user.id, amount: 0, status: "flagged", reason: fraudReason });
    ...
    await adminClient.from("quiz_attempts").insert({
      user_id: user.id, book_id, score: correct, passed, ip_address: ipAddress, user_agent: userAgent,
    });
    await adminClient
      .from("sprint_entries")
      .update({ score: correct, time_spent_ms, submitted_at: new Date().toISOString(), status: passed ? "completed" : "failed" })
      .eq("sprint_id", sprint_id)
      .eq("user_id", user.id);
    return new Response(
  ...
  if (!isCompetition) {
    ...
    if (activeReadathon) {
      ...
      await adminClient.from("readathon_completions").insert({
        readathon_id: activeReadathon.id, user_id: user.id, square_id: square.id,
      });
      ...
      if (rowComplete) {
        ...
        if (!existingBingo) {
          await adminClient.from("readathon_bingos").insert({
            readathon_id: activeReadathon.id, user_id: user.id, row_index: square.row_index,
          });
          bingo = true;
        }
      }
  ...
  if (isCompetition) {
    ...
    await adminClient.from("elimination_progress").insert({
      competition_id, user_id: user.id, round: competition_round,
      score: correct, total_questions: total, passed, submitted_at: new Date().toISOString(),
    });
    return new Response(
  ...
  await adminClient.from("completed_books").upsert({
    user_id: user.id, book_id, passed, completed_at: new Date().toISOString(),
  });
  if (!passed) {
    return new Response(
  ...
  if (book.book_type === "sponsored") {
    const { data: rpcResult } = await adminClient.rpc("claim_bounty_payout", {
      p_book_id: book_id, p_user_id: user.id,
    });
  ...
  await adminClient.from("payout_logs").insert({
    user_id: user.id, amount: earnedAmount, status: payoutStatus, reason: "Bounty payout",
  });
  return new Response(
    JSON.stringify({ passed: true, score: correct, total, earned_amount: earnedAmount, pool_exhausted: false }),
    { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
  );
});
