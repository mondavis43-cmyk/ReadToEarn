...
const AuthorBetaReaders = () => {
  ...
  const addQuestion = () => {
    if (questions.length < MAX_QUESTIONS) setQuestions(prev => [...prev, ""]);
  };
  ...
  const handleCheckout = () => {
    if (!isFormValid()) {
      setError("Please fill in all required fields and select a package.");
      return;
    }
    setError("");
    ...
    sessionStorage.setItem("checkoutItem", JSON.stringify({
      ...
      label:  `Beta Readers — ${selectedPkg!.label} (${selectedPkg!.readers} readers) — ${form.bookTitle}`,
      amount: selectedPkg!.cents,
      metadata: {
        package: selectedPkg!.label,
        readers: String(selectedPkg!.readers),
      },
    }));
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      <div className="max-w-2xl mx-auto px-4 py-16">
        <div className="mb-10">
          <button
            onClick={() => { window.history.pushState({}, "", "/author-dashboard"); window.dispatchEvent(new PopStateEvent("popstate")); }}
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-white font-semibold text-lg">
                  Custom Questions <span className="text-gray-500 font-normal text-sm">(optional)</span>
                </h2>
          ...
          >
            {selectedPkg
              ? `Continue to Checkout — $${selectedPkg.price.toFixed(2)}`
              : "Complete all fields to continue"}
          </button>
          <p className="text-gray-600 text-xs text-center">
            Secure checkout. Your submission goes live after admin review.
          </p>
