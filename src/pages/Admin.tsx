...
import { ArrowLeft, Plus, Trash2, Check, X, Pencil, Mail, Sun, Moon } from 'lucide-react';
...
export const Admin = () => {
  ...
  const handleSaveBook = async () => {
    ...
    if (incomplete) { setError('Please complete all 10 questions.'); return; }
    setSaving(true);
    ...
    const { data: bookData, error: bookError } = await supabase
      ...
      .select()
      .single();
    if (bookError || !bookData) { setError('Failed to save book.'); setSaving(false); return; }
  ...
  const handleSaveEdit = async () => {
    ...
    if (bookError) { setError('Failed to update book.'); setSaving(false); return; }
    ...
    return (
      <div className="min-h-screen bg-[#F5F0E8] dark:bg-[#0f0f0f] transition-colors duration-200">
        ...
        <main className="max-w-5xl mx-auto px-4 py-10 space-y-6">
          ...
          {success && <p className="text-[#1B2A4A] dark:text-[#D4A843] text-sm font-medium">{success}</p>}
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-lg border border-[#e8e0d5] dark:border-gray-800 p-6 space-y-6">
            ...
            {editingQuestions.map((q, i) => (
              <div key={q.id} className="border border-[#e8e0d5] dark:border-gray-700 rounded-lg p-4 space-y-3">
                ...
                <input className={correctInputClass} placeholder="Correct Answer" value={q.correct_answer}
                  onChange={(e) => {
                    const updated = [...editingQuestions];
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8] dark:bg-[#0f0f0f] transition-colors duration-200">
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        {success && <p className="text-[#1B2A4A] dark:text-[#D4A843] text-sm font-medium mb-4">{success}</p>}
        ...
        {activeTab === 'books' && (
          <div className="space-y-4">
            {books.map((book) => (
              <div key={book.id} className="bg-white dark:bg-[#1a1a1a] rounded-lg border border-[#e8e0d5] dark:border-gray-800 p-4 flex items-center gap-4">
                {book.cover_url && (
                  <img src={book.cover_url} alt={book.title} className="w-12 h-16 object-cover rounded" />
                )}
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-[#1B2A4A] dark:text-[#F5F0E8] truncate">{book.title}</p>
                  <p className="text-[#2C2C2C]/50 dark:text-gray-400 text-sm">{book.author}</p>
                  <p className="text-[#2C2C2C]/40 dark:text-gray-500 text-xs">{book.page_count} pages · Bounty: ${book.bounty_amount.toFixed(2)}</p>
                </div>
            ...
            {!showAddForm ? (
              ...
              <div className="bg-white dark:bg-[#1a1a1a] rounded-lg border border-[#e8e0d5] dark:border-gray-800 p-6 space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setShowAddForm(false)} className="text-[#2C2C2C]/40 dark:text-gray-500 hover:text-[#2C2C2C] dark:hover:text-[#F5F0E8] transition">
                    <X className="w-4 h-4" />
                  </button>
                </div>
        ...
        {activeTab === 'cashouts' && (
          <div className="space-y-3">
            {cashouts.length === 0 && (
              <p className="text-[#2C2C2C]/50 dark:text-gray-500 text-sm">No cashout requests yet.</p>
            )}
            {cashouts.map((c) => (
              <div key={c.id} className="bg-white dark:bg-[#1a1a1a] rounded-lg border border-[#e8e0d5] dark:border-gray-800 p-4">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    <p className="font-medium text-[#1B2A4A] dark:text-[#F5F0E8] text-sm">{c.profiles?.email}</p>
                    <p className="text-[#2C2C2C]/50 dark:text-gray-500 text-xs mt-0.5">
                      {new Date(c.created_at).toLocaleString()} · {c.payout_type}
                    </p>
                    <p className="text-[#2C2C2C]/60 dark:text-gray-400 text-xs mt-1">{c.payout_details}</p>
                    {c.gift_card_brand && (
                      <p className="text-[#2C2C2C]/50 dark:text-gray-500 text-xs">Brand: {c.gift_card_brand}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-[#1B2A4A] dark:text-[#F5F0E8] font-semibold">${c.amount.toFixed(2)}</span>
        ...
        {activeTab === 'waitlist' && (
          <div className="space-y-3">
            {waitlist.length === 0 && (
              <p className="text-[#2C2C2C]/50 dark:text-gray-500 text-sm">No waitlist entries yet.</p>
            )}
            {waitlist.map((entry) => (
              <div key={entry.id} className="bg-white dark:bg-[#1a1a1a] rounded-lg border border-[#e8e0d5] dark:border-gray-800 p-4 flex items-center gap-3">
                <Mail className="w-4 h-4 text-[#1B2A4A]/40 dark:text-gray-500 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[#1B2A4A] dark:text-[#F5F0E8] text-sm font-medium truncate">{entry.email}</p>
                  <p className="text-[#2C2C2C]/40 dark:text-gray-600 text-xs">{new Date(entry.created_at).toLocaleDateString()}</p>
                </div>
