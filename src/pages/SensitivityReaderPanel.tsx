...
interface SensitivityPanel {
  ...
  chapter_text: string;
  excerpt: string | null;
  identity_areas: string[];       // normalized to array always
  questions: Question[];
...
// identity_requirements is a single string in sensitivity_panels
// identity_areas is an array in author_sensitivity_submissions
function normalizeIdentities(raw: any): string[] {
  if (Array.isArray(raw)) return raw;
  if (typeof raw === 'string' && raw.trim()) return [raw];
  return [];
}

function normalizeAuthorPanel(p: any): SensitivityPanel {
  return {
    id: p.id,
    source: 'author',
    author_name: p.author_name ?? '',
    title: p.book_title ?? p.title ?? '',
    context: p.context ?? null,
    chapter_text: p.chapter_text ?? '',
    excerpt: p.excerpt ?? null,
    identity_areas: normalizeIdentities(p.identity_areas),
    questions: parseQuestions(p.custom_questions),
    max_responses: p.readers ?? p.max_responses ?? 0,
    responses_count: p.completions_count ?? p.responses_count ?? 0,
    payout_per_response: p.payout_per_response ?? DEFAULT_PAYOUT,
    notes: p.notes ?? null,
  };
}

function normalizeAdminPanel(p: any): SensitivityPanel {
  return {
    id: p.id,
    source: 'admin',
    author_name: p.author_name ?? '',
    ...
    chapter_text: p.chapter_text ?? '',
    excerpt: p.excerpt ?? null,
    identity_areas: normalizeIdentities(p.identity_requirements),
    questions: parseQuestions(p.questions),
    max_responses: p.max_responses ?? 0,
...
export default function SensitivityReaderPanel() {
  ...
  async function loadPanels() {
    ...
    const authorPanels = (authorData ?? [])
      .map(normalizeAuthorPanel)
      .filter((p) => p.responses_count < p.max_responses);

    const adminPanels = (adminData ?? [])
      .map(normalizeAdminPanel)
      .filter((p) => p.responses_count < p.max_responses);

    const all = [...authorPanels, ...adminPanels];
    ...
    if (user) {
      const { data: done } = await supabase
        ...
        .eq('user_id', user.id);
      setCompletedIds(new Set((done ?? []).map((r: any) => r.submission_id)));
    }
  ...
  function isValid() {
    ...
    const requiredAnswered = selected.questions
      .filter((q) => q.required)
  ...
  async function handleSubmit() {
    ...
    const capacityCol = selected.source === 'admin' ? 'max_responses' : 'readers';
    ...
    await supabase.from('payout_logs').insert({
      ...
      amount: earned,
      status: 'completed',
      reason: `Sensitivity reader response - ${selected.title}`,
    });
        ...
        {panels.length === 0 ? (
          ...
          <div className="space-y-4">
            {panels.map((panel) => {
              ...
              const remaining = panel.max_responses - panel.responses_count;
              const payout = panel.payout_per_response;
              return (
                <div key={panel.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {panel.identity_areas.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-3">
                          {panel.identity_areas.slice(0, 5).map((id) => (
                            <span
                              key={id}
                          ...
                          {panel.identity_areas.length > 5 && (
                            <span className="text-xs text-gray-500 px-2 py-0.5">
                              +{panel.identity_areas.length - 5} more
                            </span>
                          )}
                        </div>
                      )}
  ...
  if (step === 'read' && selected) {
    return (
      <div className="min-h-screen bg-gray-950 text-white px-4 py-10 max-w-2xl mx-auto">
        ...
        {selected.identity_areas.length > 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-5">
            <p className="text-xs text-gray-500 uppercase tracking-wide mb-2">
              Seeking feedback from
            </p>
            <div className="flex flex-wrap gap-1">
              {selected.identity_areas.map((id) => (
                <span
                  key={id}
        ...
        {selected.notes && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-5">
            <p className="text-xs text-gray-500 uppercase tracking-wide mb-2">Notes from author</p>
            <p className="text-gray-300 text-sm leading-relaxed">{selected.notes}</p>
          </div>
        )}

        {/* Show excerpt first if available, then full chapter */}
        {selected.excerpt && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-5">
            <p className="text-xs text-gray-500 uppercase tracking-wide mb-4">Excerpt</p>
  ...
  if (step === 'answer' && selected) {
    const payout = selected.payout_per_response;
    return (
      <div className="min-h-screen bg-gray-950 text-white px-4 py-10 max-w-2xl mx-auto">
        <button
