...
export const AuthorAMA = () => {
  ...
  return (
    <div
      className={`min-h-screen ${isDark ? 'bg-[#0F1923]' : 'bg-[#FAF8F5]'}`}
    >
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className={`text-3xl font-bold ${textPrimary} mb-3`}>
            Ask the Author
          </h1>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <ChevronRight
                      size={18}
                      className="text-[#D4A843] shrink-0 mt-1 group-hover:translate-x-1 transition-transform"
                    />
                  </div>
                </button>
              );
...
Now AMASession.tsx — the individual session page:
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#0F1923]' : 'bg-[#FAF8F5]'}`}>
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        {/* Submit question (open status, not author, one per user) */}
          ...
          <div className={`rounded-2xl border ${cardBg} p-5 mb-6`}>
            ...
            {questionSuccess ? (
              ...
              <p className={`text-sm ${textMuted}`}>
                You've already submitted a question for this AMA.
              </p>
            ) : !user ? (
              <p className={`text-sm ${textMuted}`}>
                <button onClick={() => navigateTo('/login')} className="text-[#D4A843] underline">Sign in</button> to submit a question.
              </p>
            ) : (
              <>
                <textarea
                ...
                {questionError && (
                  <p className="text-xs text-red-500 mt-2">{questionError}</p>
                )}
              </>
            )}
          </div>
            ...
            questions.map((q) => {
              ...
              return (
                <div key={q.id} className={`rounded-2xl border ${cardBg} overflow-hidden`}>
                  ...
                  {/* Author answer box (author only, no answer yet, answering status) */}
                  ...
                  {answer && (
                    <div className="border-t border-[#D4A843]/20 bg-[#D4A843]/5 p-5">
                      ...
                      {/* Reply box (question owner only, after answer) */}
