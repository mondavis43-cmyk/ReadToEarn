...
export function AdminSprints() {
  ...
  const handleCloseAndPay = async (id: string, title: string) => {
    ...
    setPayingOut(null);
    if (rpcError) { setError(`Payout failed: ${rpcError.message}`); return; }
    if (data === 'already_completed') { setError('This sprint has already been paid out.'); return; }
    if (data === 'no_prize_pool') { setError('No prize pool to distribute.'); return; }
  ...
  const handleCreate = async () => {
    setError('');
    if (!form.title || !form.book_id || !selectedBook || (!form.is_sponsored && !form.entry_fee) || !form.start_date || !form.end_date) {
    ...
    <div className="space-y-6">
      ...
      {success && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-3 text-sm text-green-600 dark:text-green-400 flex items-center justify-between">
          {success}
          <button onClick={() => setSuccess('')} className="ml-3 text-green-400 hover:text-green-600"><X size={14} /></button>
        </div>
      )}
        ...
        <div className="bg-white dark:bg-gray-800 border border-[#e8e8d5] dark:border-gray-700 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              />
              {selectedBook && (
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#D4A843]/10 border border-[#D4A843]/30 text-sm mt-1">
              ...
              {books.length > 0 && !selectedBook && (
                <div className="border border-[#e8e8d5] dark:border-gray-600 rounded-xl overflow-hidden max-h-48 overflow-y-auto mt-1">
                  {books.map((b) => (
                    <button
                      key={b.id}
                      onClick={() => {
      ...
      {sprints.length === 0 && !creating ? (
        ...
        <div className="space-y-4">
          {sprints.map((s) => {
            ...
            return (
              <div key={s.id} className="bg-white dark:bg-gray-800 border border-[#e8e8d5] dark:border-gray-700 rounded-2xl overflow-hidden">
